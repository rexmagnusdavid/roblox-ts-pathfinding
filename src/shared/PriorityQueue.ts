/**
 * Credits: I have unfortunately lost the exact library where I snagged this PriorityQueue class, but implementing one should be self-explanatory.
 *
...
export class PriorityQueue<T extends defined> {
	 ...
	 */
	public top() {
		return this._queue.size() > 0 ? this._queue[0] : undefined;
	}
	...
	public pop() {
		...
		this._queue[0] = this._queue[this._queue.size() - 1];
		this._swap(0, this._queue.size() - 1);
		this._queue.pop();
		this._heapify(0);
		return item;
	}
	...
	public contains(value: T, comparator?: (item: T) => boolean) {
		...
		const mid = math.floor(this._queue.size() / 2);
		...
		while (index <= mid - 1) {
			...
			if (
				(this._queue[index] === undefined && func(this._queue[index]) === undefined) ||
				(this._queue[childIndex1] === undefined && func(this._queue[childIndex1]) === undefined) ||
				(this._queue[childIndex2] === undefined && func(this._queue[childIndex2]) === undefined)
			) {
				return true;
			}
